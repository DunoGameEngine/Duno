...
GLShadowRenderer::GLShadowRenderer()
{
	...
	float size = 20;
	m_shadow_projection_matix = glm::ortho(-size, size, size, -size);//glm::perspective(70.0F, 1280.0F / 720.0F, 0.1F, 1000.0F);
	m_shadow_cam = new DunoCamera(glm::vec3(), glm::vec3(90, 0, 0), glm::vec3(1, 1, 1));
}

void GLShadowRenderer::onRenderModel(DunoGameObject* model, DunoCamera* cam)
{
	m_shadow_cam->setPosition(glm::vec3(cam->getPosition().x, 0, cam->getPosition().z));// + glm::vec3(0, -20, 0));
	m_shadow_cam->setRoation(glm::vec3(90, 0, 0));
